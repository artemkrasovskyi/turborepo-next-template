...
import { loadMoreLikedPostsAction } from '../actions';

type LikedPostsLoadMoreButtonProps = {
  userId: string;
  viewerId: string | null;
  initialCursor: string | null;
};

export const LikedPostsLoadMoreButton: FC<LikedPostsLoadMoreButtonProps> = ({
  userId,
  viewerId,
  initialCursor,
}) => {
  ...
  const handleLoadMore = () => {
    ...
    startTransition(async () => {
      try {
        const page = await loadMoreLikedPostsAction(userId, cursor!, viewerId ?? undefined);
